...
- **Orphaned core shadcn tokens** (`popover`, `popover-foreground`, `accent`, `accent-foreground`, `border`, `input`, `ring`) — these are consumed by shadcn primitives (Radix dropdowns, popovers, scroll-areas) directly via Tailwind classes, not through DESIGN.md component entries. We document them so the contract is complete; they're not orphaned in the codebase.
